...
<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {//ถ้ากดส่ง
        ...
        if (empty($_POST["txt_keyword"])) {//กดenterแต่ไม่ใส่ข้อความ
            // $str_sql = "SELECT * FROM Order_Blood WHERE Order_ID = 0 AND Status = 1 OR Status = 2";//เพื่อให้หาไม่เจอ(0 เพราะวเรากำหนดIDเริ่มจาก1)
            $str_sql = "SELECT * FROM Order_Blood WHERE `Status` = 1 OR `Status` = 2 AND Endorser_ID = ".$_SESSION['Employee_ID'];
        } else {
            $txt_keyword = $_POST["txt_keyword"];//
            $str_sql = "SELECT * FROM Order_Blood WHERE Order_ID LIKE '%$txt_keyword%' OR Endorser_ID LIKE '%$txt_keyword%' AND  Status = 1 OR Status = 2 AND Endorser_ID = ".$_SESSION['Employee_ID'];
...
<nav class="nav nav-pills nav-fill">
        ...
        <a class="nav-link active " href="StatusOrderMember.php">StatusOrder</a>
